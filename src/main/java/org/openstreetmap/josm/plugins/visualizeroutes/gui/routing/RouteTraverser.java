...
import org.openstreetmap.josm.plugins.visualizeroutes.constants.OsmTurnRestrictionRelationTags;
...
import org.openstreetmap.josm.plugins.visualizeroutes.gui.routing.transportmode.RouteTypes;
import org.openstreetmap.josm.plugins.visualizeroutes.gui.utils.StopAreaUtils;
import org.openstreetmap.josm.plugins.visualizeroutes.utils.GeometryUtils;
...
public class RouteTraverser {
    ...
    private List<List<RelationMember>> groupByStop() {
        ...
        for (RelationMember m : this.relation.getMembers()) {
            if (OsmRouteRelationTags.STOP_AND_PLATFORM_ROLES.contains(m.getRole())) {
                Relation area = StopAreaUtils.findContainingStopArea(m.getMember());
                if (area != null && area == lastArea) {
                    ...
                    areaMembers.add(OsmRouteRelationTags.STOP_ROLES.contains(m.getRole()) ? 0 : areaMembers.size(), m);
                } else {
    ...
    private PointOnRoute findCloseToRoute(LatLon point) {
        ...
        bounds.add(point);
        bounds = GeometryUtils.increaseSize(bounds, MAX_SIDEWARD_DISTANCE);
    ...
    private List<RouteSegment> findSegments() {
        List<Way> ways = getSegmentMembers()
            .filter(rm -> OsmRouteRelationTags.ROLES_SEGMENT.contains(rm.getRole()))
    ...
    private Stream<Relation> findTurnRestrictions(Way way, String prefix) {
        return way.referrers(Relation.class)
            .filter(r -> r.hasTag(OsmTurnRestrictionRelationTags.KEY_TYPE, OsmTurnRestrictionRelationTags.KEY_TYPE_VALUE_TURN_RESTRICTION))
            .filter(r -> {
    ...
    private Stream<RelationMember> getSegmentMembers() {
        if (relation.hasTag(OsmRouteRelationTags.KEY_TYPE, OsmRouteRelationTags.KEY_TYPE_VALUE_ROUTE)) {
            return relation.getMembers()
                .stream();
        } else if (relation.hasTag(OsmRouteRelationTags.KEY_TYPE, "superroute")) {
            return relation
                .getMembers()
                .stream()
